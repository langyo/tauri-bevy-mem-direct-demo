...
fn create_webview2_environment() -> ICoreWebView2Environment {
    ...
    CreateCoreWebView2EnvironmentCompletedHandler::wait_for_async_operation(
        ...
        Box::new(move |error_code, environment| {
            error_code?;
            tx.send(environment)
                .expect("Failed to send webview2 environment");
            Ok(())
        }),
    )
    ...
    rx.recv()
        .expect("Failed to receive webview2 environment")
...
"#;
        ...
        // Upstream wry does not expose environment(); create a parallel environment for SharedBuffer.
        let env = create_webview2_environment();
